...
if (array_key_exists('userid', $_SESSION)){	//If user is logged, check for access level 
	if($_SESSION['access']=='S'){
		...
		if(isset($_POST['_del_submit'])){ //Edit user form has been submitted so it's time to update the database
			//include 'dump_all_page_contents.php';
			//die();
			//"{$_REQUEST['user_id']}"
			$query = "DELETE FROM users WHERE user_id='{$_POST['user_id_to_delete']}'"; 
			$result = mysqli_query($db,$query);
			
			if($result){
				//show success message
				print '<h3 align="center"> User Deleted!</h3>';
				//print '<h4 align="center"><a href="admin.php">Return to Admin Page</a></h4>';
			}else{
				echo 'The system encountered a problem deleting the user specified, please contact your administrator';
			}
			
		}
			
		//once user is authenticated, check to see if this form has been submitted
		if(isset($_POST['_edit_submit'])){ //Edit user form has been submitted so it's time to update the database
			//check for errors
			if(validateUserChanges()=='valid_withPass') //if no errors, update user in db and show success message
			{
				...
				}else{ //still update everything else, but not the password--otherwise you will overwrite your password with a blank space
				...
				print '<h4 align="center"><a href="edituser.php">Return to User List</a></h4>';
